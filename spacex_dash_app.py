...
def update_scatter_chart(entered_site, slider_range):
    ...
    f_df = spacex_df[(spacex_df['Payload Mass (kg)'] > low) & (spacex_df['Payload Mass (kg)'] < high)]
    if entered_site != 'ALL':
        f_df = f_df[f_df['Launch Site'] == entered_site]
    fig = px.scatter(f_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
    return fig
